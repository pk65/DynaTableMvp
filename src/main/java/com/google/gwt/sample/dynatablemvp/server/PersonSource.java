...
//import static com.google.gwt.sample.dynatablemvp.shared.DynaTableRequestFactory.SchoolCalendarRequest.NO_DAYS;
...
//	private static final Logger logger = Logger.getLogger(PersonSource.class.getName());
...
/*		final Person randomPerson = PersonFuzzer.generatePerson();
		if(randomPerson!=null){
			person.setMentor(randomPerson);
			logger.info(person.getName()+": (mentor: "+person.getMentor().getName()+")");
		} else
			logger.info(person.getName()+": (mentor: null)");*/
      return person;
    }

    @Override
    public List<Person> getPeople(int startIndex, int maxCount,
    		WeekDayStorage daysFilter) {
...
/*		if(toReturn.getAddress()!=null)
			logger.info("{"+toReturn.getName()+"}: "+toReturn.getAddress().getCity());
		else
			logger.info("{"+toReturn.getName()+"}: [null]");*/
